  ...
  const billingAddress = (payload2) => {
    ...
    res
      ...
      .catch((error) => {
        console.error(error);
        if (error?.response?.data?.error) {
          setBillingPinCodeError(error?.response?.data?.error);
        }
        if(error?.response?.data?.error === "Location already exist, please use a different location or update existing one!!") {
          refreshData();
        }
      });
  };
  const handlePinCodeErrorReset = (e) => {
    if (e.target.value) {
      setPincodeError("");
    }
  };
  const handleBillingPinCodeError = (e) => {
    if (e.target.value) {
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item md={6} xs={12}>
                    <TextField
                      ...
                      onKeyUp={handleBillingPinCodeError}
                    />
                  </Grid>
                  <Grid item md={6} xs={12}>
                    <DropDown
                  ...
                  {billingPinCodeError && <Grid item md={12} xs={12}>
                    <FormHelperText sx={{ color: error.main }}>
                      {"* "}{billingPinCodeError === "PINCODE is invalid" ? "Please enter valid pin code" : billingPinCodeError}
                    </FormHelperText>
                  </Grid>}
                </Grid>
              )}
...
const checkoutSchema = yup.object().shape({
  ...
  billing_contact_number: yup.string().when("same_as_shipping", {
    is: false,
    then: yup.string().required("Please enter the number.")
      .min(1)
      .max(10, "contact must be at most 10 digits")
      .matches(
        /^((\(\d{3}\)[ |-]?|\d{3}-)\d{3}-\d{4}|\d{10})$/,
